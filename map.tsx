...
export default function App(): JSX.Element {
  ...
  const fetchNearbyPlaces = async (latitude: number, longitude: number) => {
    try {
      const types = ['police', 'pharmacy', 'convenience_store', 'hospital', 'bank', 'toilet'];
  ...
  const fetchRoute = async (startLat: number, startLng: number, endLat: number, endLng: number) => {
    try {
      ...
      if (response.data.routes.length > 0) {
        ...
        const routeCoords = decodedPoints.map(([lat, lng]: [number, number]) => ({
          latitude: lat,
          longitude: lng,
        }));
  ...
  const handlePoliceStationPress = () => {
    if (nearbyPlaces.length > 0) {
      const nearestPolice = nearbyPlaces
        ...
        .reduce((prev, curr) => (prev.distance && curr.distance && prev.distance < curr.distance ? prev : curr), nearbyPlaces[0]);
...
const handlestorePress = () => {
  if (nearbyPlaces.length > 0) {
      const nearestsore = nearbyPlaces
      .reduce((prev, curr) => (prev.distance && curr.distance && prev.distance < curr.distance ? prev : curr), nearbyPlaces[0]);
    ...
    return (
      <TouchableOpacity onPress={() => handlePlacePress(item)}>
        <View style={styles.placeItem}>
          <Image source={getPlaceIcon(item.type)} style={{ width: 30, height: 30, marginRight: 10 }} />
          <Text style={styles.placeName}>{item.name}</Text>
          <View>
            <Text style={styles.placeDistance}>현 위치에서 {item.distance}m ⭢</Text>
          </View>
        </View>
      </TouchableOpacity>
    );
      ...
      <View style={styles.container}>
        <GooglePlacesAutocomplete
          ...
          onPress={(data, details = null) => {
            if (details) {
              const { lat, lng }: { lat: number; lng: number } = details.geometry.location;
        ...
        <TouchableOpacity style={styles.myLocationButton} onPress={handleCenterMap}>
          <Image source={require('../../assets/images/icon/star.png')} style={styles.buttonImage} />
          <Text style={styles.rapidstar}>현재 위치</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.policeButton} onPress={handlePoliceStationPress}>
          <Image source={require('../../assets/images/icon/police.png')} style={styles.buttonImage} />
          <Text style={styles.rapidstar}>경찰</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.heartBTN} onPress={handleHospitalPress}>
          <Image source={require('../../assets/images/icon/heart.png')} style={styles.buttonImage} />
          <Text style={styles.rapidstar}>병원</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.pharmacytBTN} onPress={handlepharmacyPress}>
          <Image source={require('../../assets/images/icon/pharmacy.png')} style={styles.buttonImage} />
          <Text style={styles.rapidstar}>약국</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.coinBTN} onPress={handlebankPress}>
          <Image source={require('../../assets/images/icon/coin.png')} style={styles.buttonImage} />
          <Text style={styles.rapidstar}>은행</Text>
        </TouchableOpacity>

        <TouchableOpacity style={styles.storeBTN} onPress={handlestorePress}>
          <Image source={require('../../assets/images/icon/store.png')} style={styles.buttonImage} />
          <Text style={styles.rapidstar}>상점</Text>
        </TouchableOpacity>

        <BottomSheet ref={sheetRef} snapPoints={snapPoints} enablePanDownToClose={false}>
