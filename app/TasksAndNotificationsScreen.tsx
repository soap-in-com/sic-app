...
const TasksAndNotificationsScreen: React.FC = () => {
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    {
      id: 1,
      message: '가스점검: 불을 꼭 꺼주세요!',
      isChecked: true,
      color: '#32CD32',
    },
    {
      id: 2,
      message: '병원 방문 시 주민등록증 챙기기',
      isChecked: false,
      color: '#32CD32',
    },
  ...
  const loadTodayTasks = async () => {
    try {
      ...
      if (storedData !== null) {
        ...
        // 오늘의 데이터를 찾아 일정만 필터링
        const todayData = parsedData.find(
          (day: DayData) => day.date === todayFormattedDate
        );
        ...
        <View style={styles.cardContainer}>
          ...
          >
            ...
            {tasks.length > 0 ? (
              tasks.slice(0, 3).map((task, index) => (
                <TouchableOpacity
                  key={task.id ? task.id.toString() : index.toString()}
          ...
          >
            ...
            {notifications.slice(0, 3).map((notif, index) => (
              <View
                key={notif.id ? notif.id.toString() : index.toString()}
                style={styles.item}
              >
                <CheckBox
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            <ScrollView>
              ...
              {(selectedType === 'notifications' ? notifications : tasks).map(
                (item) =>
                  isNotification(item) ? (
                    <TouchableOpacity
                      key={item.id?.toString()}
                      onPress={() => toggleNotificationChecked(item.id)}
                      style={styles.item}
                    >
                      <CheckBox
                      ...
                      >
                        {item.message || '내용을 불러올 수 없습니다.'}
                      </Text>
                    </TouchableOpacity>
                  ) : (
                    <TouchableOpacity
                      key={item.id?.toString()}
                      onPress={() => toggleTaskChecked(item.id)}
                      style={styles.item}
                    >
                      <CheckBox
                        value={item.isChecked}
