...
const ScheduleAndMedicineScreen: React.FC = () => {
  ...
  const scrollToTodayMedicine = () => {
    if (todayIndex !== -1 && medicineFlatListRef.current) {
      medicineFlatListRef.current.scrollToIndex({
        index: todayIndex,
        animated: true,
      });
    }
  ...
  const scrollToTodaySchedule = () => {
    if (todayIndex !== -1 && scheduleFlatListRef.current) {
      scheduleFlatListRef.current.scrollToIndex({
        index: todayIndex,
        animated: true,
      });
    }
  ...
  const handleSaveMemo = async (memoData: {
    ...
    if (targetIndex !== -1) {
      ...
      const maxId = newData[targetIndex].memos.reduce(
        (max, memo) => Math.max(max, memo.id),
        0
      );
      const newMemo = {
